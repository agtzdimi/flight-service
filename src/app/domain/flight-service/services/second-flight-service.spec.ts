...
beforeEach(async () => {
  const testModule = await getUnitTestingModule(SecondFlightService);
...
test('fetchFlights (flight service 2) - returns valid flights', async () => {
  const mockResponse = {
    flights: [
      {
        slices: [
  ...
  const returned = await secondFlightService.fetchFlights();

  expect(returned.length).toBe(2);

  const expectedUrl = `https://coding-challenge.powerus.de/flight/source2`;
...
test('fetchFlights (flight service 2) - returns empty array on not valid flights', async () => {
  const mockResponse = {
    flights: [
      {
        slices: [
...
test('fetchFlights (flight service 2) - returns empty array on status code 500 from service', async () => {
